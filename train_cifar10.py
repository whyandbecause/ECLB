...
parser.add_argument('--root', default='/media/DATA3/Data/coderv/dataset/CIFAR10/', type=str)
parser.add_argument('--num_class', default=10, type=int)
parser.add_argument('--noise_type', default='sym', type=str)#sym， asym
parser.add_argument('--noise_ratio', default=0.4, type=float)
parser.add_argument('--sim', default=0.5, type=float)
...
parser.add_argument('--save_dir', default="./checkpoints/exp3_cifar10_symmetric_0.4", type=str)
...
trainset = Cifar10(root=args.root, num_class=args.num_class, mode="train", noise_type=args.noise_type, noise_ratio=args.noise_ratio)
testset = Cifar10(root=args.root, num_class=args.num_class, mode="test", noise_type='clean', noise_ratio=0)
...
if __name__ == "__main__":
    ...
    if not os.path.exists(y_init_path):
        ...
        name_label_dict = {}
        for batch_idx, (_, label, file_name,_) in enumerate(train_dataloader_temp):
            #print(file_name[0])
    ...
    for epoch in range(args.epoch):
        ...
        for i, data in enumerate(trainloader, 0):
            with autograd.detect_anomaly():
                ...
                inputs, labels, file_names,_ = data
                bsz = labels.size(0)
                ...
                for index in range(len(feat_list)):
                    ...
                    mask &= (similarity>args.sim)#其他无后缀文件夹用的阈值为0.6
                    #mask |= (similarity>0.8)
                    if args.supervision:
